...
module.exports = new CommandInterface({
	...
	execute: async function(p){
		let args=p.args,msg=p.msg,con=p.con;
		...
		if(result[1][0]){
			...
			if(amount=="all"){
				if(money<=0)
					...
					await initBlackjack(p,money);
			}else{
				if(money<amount)
					p.send("**🚫 | "+msg.author.username+"**, You do not have enough cowoncy!",3000);
				else
					await initBlackjack(p,amount);
			}
		}else{
			p.send("**🚫 | "+msg.author.username+"**, You do not have enough cowoncy!",3000);
		}
...
async function blackjack(p,player,dealer,bet,resume){
	...
	let filter = (emoji, userID) => (emoji.name === hitEmoji ||emoji.name === stopEmoji) && userID === p.msg.author.id;
	...
	collector.on('end',(collected,reason) => {
		if(reason=="time") message.edit("**⏱ |** This session has expired. Retype `owo blackjack` to resume");
	});
}
...
async function hit(p,player,dealer,msg,bet,collector){
	...
	if(ppoints>21){
		...
		let sql = "INSERT INTO blackjack_card (bjid,card,dealer,sort) VALUES ((SELECT bjid FROM blackjack WHERE id = "+p.msg.author.id+"),"+card.card+",0,"+player.length+") ON DUPLICATE KEY UPDATE dealer = 0,sort= "+player.length+";";
...
async function stop(p,player,dealer,msg,bet,fromHit){
	...
	let sql2 = "DELETE FROM blackjack_card WHERE bjid = (SELECT bjid FROM blackjack WHERE id = "+p.msg.author.id+");";
	if(winner=='w')
...
async function parseQuery(info){
	if(info.query){
		...
		let sql = "SELECT * FROM blackjack LEFT JOIN blackjack_card ON blackjack.bjid = blackjack_card.bjid WHERE id = "+info.id+" AND active = 1 ORDER BY sort ASC ,dealer DESC;";
